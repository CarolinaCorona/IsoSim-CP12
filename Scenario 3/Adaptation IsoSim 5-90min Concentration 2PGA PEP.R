...
cat("
    This code calculates the flux through 2PG <-> PEP in 6 Synechocystis strains with concentrations 5-90 mins
    \n \n")
...
setwd(wd)
if (!file.exists("Run-2pg-pep-10-concentrations-5-90-upper100")){
  dir.create(file.path(wd, "Run-2pg-pep-10-concentrations-5-90-upper100"))
}
setwd(file.path(wd, "Run-2pg-pep-10-concentrations-5-90-upper100"))
...
times <- c(5,10,15,30,60,90)

noise <- 0.0005

# 13C-enrichments of PG2 (label input) in strains "dcp12", "dcp12::cp12", "dcp12::cp12dCysC", "dcp12::cp12dCysN", "dcp12::cp12dCysNC", "WT K"
original_data_input <- cbind("WTK_PG2_1-2-3"=c(0, 0, 0, 0, 0, 0),
                             "dcp12_PG2_1-2-3"=c(0, 0.0010236875, 0.005175985, 0.1018379825, 0.2487054925, 0.3470264875),
                             "dcp12cp12_PG2_1-2-3"=c(0, 0, 0, 0, 0.0002532375, 0.003050705),
                             "dcp12cp12dCysC_PG2_1-2-3"=c(0, 0, 0.0067180525, 0.0495900833333333, 0.0565733, 0.0485604875),
                             "dcp12cp12dCysN_PG2_1-2-3"=c(0, 0, 0, 0, 0, 0),
                             "dcp12cp12dCysNC_PG2_1-2-3"=c(0, 0, 0.0111800575, 0.0767527975, 0.204328553333333, 0.24085604))
...
# 13C-enrichments of metabolic intermediates (PEP)
enr_PEP_dcp12 <- c(0, 0.007159935, 0.0217724333333333, 0.10046094, 0.2625095, 0.343587925)
...
# 13C-enrichments of metabolic intermediates (PEP)
enr_PEP_dcp12_cp12 <- c(0, 0, 0, 0, 0.004714665, 0.0074160425)
...
# 13C-enrichments of metabolic intermediates (PEP)
enr_PEP_dcp12_cp12dCysC <- c(0.1172929425, 0.00006518, 0.0141154775, 0.05690679, 0.05690679, 0.058643385)
...
enr_PEP_dcp12_cp12dCysN <- c(0, 0, 0, 0, 0.001211588, 0.001158125)
...
# 13C-enrichments of metabolic intermediates (PG2A, PEP)
enr_PEP_dcp12_cp12dCysNC <- c(0,	0.002868095,	0.019125045,	0.085424458,	0.273779933,	0.265068757)
...
sd_dcp12 <- list("2PG"=c(0, 0.002047375, 0.007319948, 0.034679842, 0.043441443, 0.075407513),
                 "PEP"=c(0, 0.011553127, 0.012540274, 0.020043716, 0.043465652, 0.030065502)
)
...
sd_dcp12_cp12 <- list(
  "2PG"=c(0, 0, 0,	0, 0.000506475,	0.00610141),
  "PEP"=c(0, 0, 0, 0, 0.00942933, 0.012240472)
)
...
sd_dcp12_cp12dCysC <- list(
  "2PG"=c(0, 0, 0.007980192, 0.01988221, 0.011417744, 0.013267179),
  "PEP"=c(0.234585885, 0.00013036, 0.006763073, 0.011860306, 0.008635114, 0.004884389)
)
...
sd_dcp12_cp12dCysN <- list(
  "2PG"=c(0, 0, 0, 0, 0, 0),
  "PEP"=c(0, 0, 0, 0, 0.00167140000030663, 0.00231625)
)
...
sd_dcp12_cp12dCysNC <- list(
  "2PG"=c(0, 0, 0.019012441, 0.029334965, 0.023493797, 0.037075651),
  "PEP"=c(0, 0.00573619, 0.019240409, 0.025263831, 0.004137565, 0.029436832)
)
